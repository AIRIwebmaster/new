...
export const metadata: Metadata = {
  ...
  description:
    'Tailored AI literacy programs for newcomers to Canada, helping new Canadians navigate the digital landscape and build confidence with AI tools for everyday life.',
};

export default async function NewCanadiansPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations('newCanadians');
  ...
  return (
    <>
      <PageHero
        ...
        breadcrumbs={[{ label: t('breadcrumbPrograms'), href: '/programs' }, { label: t('breadcrumbNewCanadians') }]}
      />
